...
wrapSelection = function(editor, selection) {
  var attribute, ditorView, endTagSelection, newEndTagSelectRange, newStartTagSelectRange, newText, range, tag, tagRangePos, text
  tag = atom.config.get('wrapper.tag')
  attribute = atom.config.get('wrapper.attribute')
  text = selection.getText()
  tagRangePos = selection.getBufferRange()
  if (attribute === '') {
    newText = ['<', tag, '>', text, '</', tag, '>'].join('')
  } else {
    newText = ['<', tag, ' ', attribute, '="" >', text, '</', tag, '>'].join('')
  }
